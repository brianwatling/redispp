...
#ifdef _WIN32
#include <windows.h>
void sleep(size_t seconds) { Sleep(seconds * 1000); }
#endif
...
struct F {
  F() : conn(TEST_UNIX_DOMAIN_SOCKET, "password") {
...
#else
struct F {
  F() : conn(TEST_HOST, TEST_PORT, "password") {
    BOOST_TEST_MESSAGE("Set up fixture");
  }
  ~F() { BOOST_TEST_MESSAGE("Tore down fixture"); }
  Connection conn;
...
BOOST_AUTO_TEST_CASE(nullreplies) {
  ...
  MultiBulkEnumerator result =
      conn.blpop(boost::assign::list_of("notalist"), 1);
  std::string str;
...
BOOST_AUTO_TEST_CASE(sets) {
  ...
  BOOST_CHECK(result.next(&str1));
  BOOST_CHECK(result.next(&str2));
  BOOST_CHECK((str1 == "world" && str2 == "mars") ||
              (str2 == "world" && str1 == "mars"));
  ...
  BOOST_CHECK(str1 == "b");
  BOOST_CHECK(!result.next(&str2));
  BOOST_CHECK(conn.sinterStore("res", boost::assign::list_of("set1")("set2")) ==
              1);
  ...
  BOOST_CHECK(str1 == "a" || str1 == "b" || str1 == "c");
  BOOST_CHECK(!result.next(&str2));
  BOOST_CHECK(conn.sunionStore("res", boost::assign::list_of("set1")("set2")) ==
              3);
  ...
  BOOST_CHECK(str1 == "a");
  BOOST_CHECK(!result.next(&str2));
  BOOST_CHECK(conn.sdiffStore("res", boost::assign::list_of("set1")("set2")) ==
              1);
  BOOST_CHECK(conn.scard("res") == 1);
}
...
BOOST_AUTO_TEST_CASE(hashes) {
  ...
  KeyValueList fields = boost::assign::list_of(
      std::make_pair("venus", "three"))(std::make_pair("jupiter", "four"));
...
BOOST_AUTO_TEST_CASE(scripts) {
  ...
  MultiBulkEnumerator result =
      conn.scriptExists(boost::assign::list_of(sha)("notascript"));
  std::string str;
  BOOST_CHECK(result.next(&str));
  BOOST_CHECK(str == "1");
  BOOST_CHECK(result.next(&str));
